...
model = glmer(choice ~ punCond * matchRound + (1 + matchRound | subject),
              family = binomial, data = df.persist);
model.null = glmer(choice ~ punCond + matchRound + (1 + matchRound | subject),
                   family = binomial, data = df.persist);
...
df.collapsed = df.persist %>% group_by(punCond, matchRound) %>% summarize(choice.se = sd(choice) / sqrt(length(choice)), choice = mean(choice))
...
write.table(df.demo %>% select(WorkerID = subject, Bonus = bonus),
            'Bonuses - sp_v2_c.csv', row.names = FALSE, col.names = FALSE, sep = ",")

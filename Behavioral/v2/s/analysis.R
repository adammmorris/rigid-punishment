...
lastMatch = max(df$matchNum)

df.persist <- df %>% filter(matchNum == lastMatch)
...
df.collapsed = df.persist %>% group_by(stealCond, matchRound) %>% summarize(choice.se = sd(choice) / sqrt(length(choice)), choice = mean(choice))

pd <- position_dodge(0.1) # move them .05 to the left and right
ggplot(data = df.collapsed, aes(x = matchRound, y = choice, color = stealCond, group = stealCond)) +
  geom_line() +

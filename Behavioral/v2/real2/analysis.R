...
df.persist <- df %>% filter(matchRound <= 7 & ((role == 0 & oppType == 0) | (role == 1 & oppType == 2)))
...
model = glmer(choice ~ role * punCond * matchRound + (1 + matchRound | subject),
              family = binomial, data = df.persist);
model.null = glmer(choice ~ role + punCond + matchRound + (1 + matchRound | subject),
                   family = binomial, data = df.persist %>% filter(role == 0));
...
ggplot(data = df.collapsed, aes(x = matchRound, y = choice, color = punCond, group = punCond)) +
  geom_line() +
...
write.table(df.demo %>% select(WorkerID = subject, Bonus = bonus),
            'Bonuses - sp_v2_real2.csv', row.names = FALSE, col.names = FALSE, sep = ",")

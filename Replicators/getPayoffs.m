...
% ks: how long it takes thief to learn
% kp: how long it takes punisher to learn
% theta: probability that it's the thief who learns to steal (and not the
%   punisher who learns to punish)

function [payoffs] = getPayoffs(N, s, sp, c, p, ks, kp, theta)
...
payoffs(1, 5, 1) = N * s - kp * p;
payoffs(1, 5, 2) = -N * sp - kp * c;
...
payoffs(2, 5, 1) = 1/2 * kp * (s - p);
payoffs(2, 5, 2) = 1/2 * kp * (-sp - c);
...
payoffs(3, 5, 1) = 1/2 * kp * (s - p) + (N - kp) * (-p * (c <= sp) + s * (c > sp));
payoffs(3, 5, 2) = 1/2 * kp * (-sp - c) - (N - kp) * (c * (c <= sp) + sp * (c > sp));
...
% Learn to steal vs always punish --- THIS IS CHANGED FROM THESIS
%payoffs(5, 1, 1) = ks * (s - p);
%payoffs(5, 1, 2) = ks * (-sp - c);
payoffs(5, 1, 1) = N * (s - p);
payoffs(5, 1, 2) = N * (-sp - c);

% Learn to steal vs always punish theft
payoffs(5, 2, 1) = ks * (s - p);
payoffs(5, 2, 2) = ks * (-sp - c);
...
%theta = ks > kp;
payoffs(5, 5, 1) = theta * (N * s - kp * p) + (1 - theta) * ks * (s - p);
payoffs(5, 5, 2) = theta * (-N * sp - kp * c) + (1 - theta) * ks * (-sp - c);

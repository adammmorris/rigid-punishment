...
paramToVary = 'c';
paramVals = linspace(.1, 2, 2);
nParamVals = length(paramVals);

%% Run sims
% Initialize payoff matrices
payoffs_costly = zeros(length(stealBias), length(punishBias), 2, nMatches, nParamVals);
payoffs_notcostly = zeros(length(stealBias), length(punishBias), 2, nMatches, nParamVals);
    ...
    for punishGene = 1:nPunishGenes
        for thisParamVal = 1:nParamVals
            ...
            parfor thisMatch = 1:nMatches
                [~, ~, payoffs_costly(thiefGene, punishGene, :, thisMatch, thisParamVal)] = ...
                    runMatch([N s sp c p], ...
                    [lr gamma temp thiefPheno punPheno 1 agentMemory]);

                [~, ~, payoffs_notcostly(thiefGene, punishGene, :, thisMatch, thisParamVal)] = ...
                    runMatch([N s sp c p], ...
...
%% Save
save('cache_full.mat', 'payoffs_costly', 'payoffs_notcostly');